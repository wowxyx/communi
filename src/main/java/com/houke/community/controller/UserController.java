...
import com.houke.community.annotation.LoginRequired;
import com.houke.community.entity.User;
import com.houke.community.service.FollowService;
import com.houke.community.service.LikeService;
...
public class UserController implements CommunityConstant {
    ...
    public String updatePassword(String oldPassword,String newPassword,String newPassword2,Model model){
            ...
            if(oldPassword == null){
            model.addAttribute("passMsg","请输入原密码！");
            return "/site/setting";
        }
            User user = hostHolder.getUser();
            oldPassword = CommunityUtil.md5(oldPassword + user.getSalt());
            if(!user.getPassword().equals(oldPassword)){
            model.addAttribute("passMsg","与原密码不一致！");
            return "/site/setting";
        }
            if(!newPassword.equals(newPassword2)){
            model.addAttribute("passwordMsg","第二次输入的密码与新密码不一致！");
            return "/site/setting";
        }
